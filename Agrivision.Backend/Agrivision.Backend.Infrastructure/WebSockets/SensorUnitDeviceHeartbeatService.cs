...
public class SensorUnitDeviceHeartbeatService(
    IServiceScopeFactory scopeFactory,
    IWebSocketConnectionManager connectionManager,
    ILogger<IrrigationUnitDeviceHeartbeatService> logger) : BackgroundService
{
    ...
    private async Task CheckDeviceStatus()
    {
        ...
        var devices = await coreDbContext.SensorUnitDevices
            .Where(d => !d.IsDeleted)
            .ToListAsync();

        var unitsByDeviceId = await coreDbContext.SensorUnits
            .Where(u => !u.IsDeleted)
